...
import extension.game.furnidata.FloorItemDetails;
import extension.game.furnidata.WallItemDetails;
...
public class FurniDataTools {
    ...
    private void fetch(/*InvalidationListener onLoad*/) throws IOException {
        ...
        floorJson.forEach(o -> {
            ...
            nameToFloorItems.put(item.getString("classname"), new FloorItemDetails(item));
        });
...
//        onLoad.invalidated(null);
    ...
    public FloorItemDetails getFloorItemDetails(String furniName) {
        return nameToFloorItems.get(furniName);
    }
    ...
    public boolean isOneWayGate(int typeId) {
        String name = getFloorItemName(typeId);
        if (name == null) return false;
        return name.startsWith("one_way_door");
    }
}
